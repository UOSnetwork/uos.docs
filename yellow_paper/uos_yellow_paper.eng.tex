...
$$
	...
	Here $\beta$ and $\gamma$ are the coefficients with values between 0 and 1, $\boldsymbol{e}$ is unit vector, $\boldsymbol{s}$ is the stack vector, and $\boldsymbol{v}$ is the validity vector.
	...
	It remains to describe validity vector $\boldsymbol{v}$, being used in the formula (\ref{teleportation_matrix_definition}). It is invented in order to additional  botnet attack protection. It is calculated with PageRank algorithm.
	
	$$
	\boldsymbol{\hat{v}}^{n+1} = (\alpha O_T+(1-\alpha)T_T) \boldsymbol{\hat{v}}^{n}
	$$
	
	Here $O_T$ is outlink matrix, based on \textit{trust} relations, $T_T$ is teleportation matrix:
