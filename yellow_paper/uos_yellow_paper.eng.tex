...
Social index calculation is based on several pieces of data. First, the matrix $\boldsymbol{V}$ contains the information about the upvotes:
	...
	Given these matrices, we may obtain now the matrix $\boldsymbol{V'}$:
	$$
\boldsymbol{V'} = ( \boldsymbol{I} + \boldsymbol{R} ) \boldsymbol{V}
$$
	%$$
	And the matrix $\boldsymbol{U}$, that we get through the intermediate matrix $\boldsymbol{\hat{U}}$:
	$$
	\hat{U}_{ik} = max_n(P_{in} V'_{nk})
	$$
	As denoted by the matrix $\boldsymbol{V}$, weights of the individual transactions decay with time. Some accounts, however, might not have any incoming activity for a period of time, which results in the columns of zeros in the matrix $V$.  Such condition violates matrix stochasticity, that is necessary for the algorithm convergence. There are several ways to resolve this. First solution is is to fill the zero columns with equal values, while second option is to add a value of 1 to the diagonal elements of the matrix. These solutions have serious drawbacks though. In a former case, we would diffuse the account rates, such that the difference between the rates of active and inactive account will be unsigniificant. In a later case, the own activity of the account would result in the rate decrease for the user, that is against their own interests. 
	To provide effective decaying weights and preserve the matrix stochasticity, we add a special hidden account $h$, which accepts upvotes from every account. For this account:
...
$$
	...
	Unlike above mentioned solutions, our solution does not have these drawbacks and also has several advantages. First advantage is that we can now measure the activity of the network with regards to a fixed standard. We can monitor what proportion of the overall network rate is received by the hidden account, which indicates the rate of activity in the network — the higher is the rate of this account the lower is the network activity. Second, now we have a standard, relative to which we can decay our weights. Therefore, we do not lose any information about the decaying weights.
	
	
	Besides above mentioned matrices, calculations are based on the stack vector $\boldsymbol{s}$ and a validity trust weight vector $\boldsymbol{v}$, containing the priority values for every account. Normally, PageRank starts the calculation from the vector of equal weights, but in our algorithm, we modify this vector to include the system of trusts in our calculation. This is our innovation, that helps further protect the network from the botnets.
	
	Based on these data structures, we may calculate social activity vectors for the accounts $\boldsymbol{\sigma^a}$ and content $\boldsymbol{\sigma^c}$. These vectors are normalized to 1:
	...
	We use the PageRank algorithm to calculate $\boldsymbol{\sigma^a}$:
	...
	Here $\beta$ and $\gamma$ are the coefficients with values between 0 and 1, $\boldsymbol{e}$ is а unit vector, $\boldsymbol{s}$ is the stack vector, and $\boldsymbol{v}$ is the validity vector.
	...
	Finally, we must remove the rating vector of the "hidden'' account. This operation changes norm of the rating vector, so we must normalize it to 1:
	...
	It remains to describe the validity vector $\boldsymbol{v}$, being used here in the formula (\ref{teleportation_matrix_definition}). As mentioned before, it is invented in order to implement additional botnet attack protection and is calculated using the PageRank algorithm.
	
	$$
	\boldsymbol{\hat{v}}^{n+1} = (\alpha O_T+(1-\alpha)T_T) \boldsymbol{\hat{v}}^{n}
	$$
	
	Here $O_T$ is outlink matrix, based on \textit{trust} relations, $T_T$ is the teleportation matrix:
